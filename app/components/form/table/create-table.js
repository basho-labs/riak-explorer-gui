...
export default Ember.Component.extend(ScrollReset, {
  ...
  actions: {
    ...
    addPartitionKey: function(type) {

    },

    addNewPartionKeyField: function(type) {
      let availableFieldNames = this.get('tableFields').mapBy('name').filter(function(name) { return Ember.isPresent(name); });

      if (Ember.isPresent(availableFieldNames)) {
